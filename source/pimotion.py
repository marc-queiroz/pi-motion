...
def captureTestImage(settings, width, height):
    command = "/opt/vc/bin/raspistill%s -w %s -h %s -t 200 -e bmp -n -o -" % (settings, width, height)
    # print(command)
...
def saveImage(settings, width, height, quality, diskSpaceToReserve):
    ...
    if use_picamera:
      ...
      subprocess.call("/opt/vc/bin/raspistill %s -w %s -h %s -t 200 -e jpg -q %s -n -o %s" % (settings, width, height, quality, filename), shell=True)
